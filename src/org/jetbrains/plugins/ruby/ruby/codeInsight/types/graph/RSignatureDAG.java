...
public class RSignatureDAG {
    ...
    public void addAll(@NotNull final List<RSignature> signatures) {
        signatures.stream()
                .map(signature -> Pair.create(signature, SqliteRSignatureCacheManager.calcArgsTypeNamesDistance(myProject,
                                                         signature.getArgsTypeName(), myRoot.getVertex().getArgsTypeName())))
    ...
    private void add(@NotNull final Node<RSignature> currentNode, @NotNull final Node<RSignature> newNode) {
        final List<Edge<RSignature>> edges = currentNode.getEdges().stream()
                .filter(edge -> {
                    ...
                    return SqliteRSignatureCacheManager.calcArgsTypeNamesDistance(myProject, argsTypeNameFrom, argsTypeNameTo) != null;
                })
        ...
        final int weight = SqliteRSignatureCacheManager.calcArgsTypeNamesDistance(myProject, argsTypeNameFrom, argsTypeNameTo);
    ...
    private boolean tryToMergeChildrenWithNode(final @NotNull Node<RSignature> currentNode,
        ...
        for (final Edge<RSignature> edge : edges) {
            ...
            if (!mergedSignature.equals(currentNode.getVertex())) {
                ...
                @SuppressWarnings("ConstantConditions")
                final int weight = SqliteRSignatureCacheManager.calcArgsTypeNamesDistance(myProject, mergedArgsTypeName,
                                                                                          currentArgsTypeName);
    ...
    private void copyChildren(@NotNull final Node<RSignature> from, @NotNull final Node<RSignature> to) {
        for (final Edge<RSignature> edge : from.getEdges()) {
            ...
            @SuppressWarnings("ConstantConditions")
            final int weight = SqliteRSignatureCacheManager.calcArgsTypeNamesDistance(myProject,
                                                                                      fromArgsTypeName, toArgsTypeName);
    ...
    private RSignature mergeSignatures(@NotNull final RSignature signature1, @NotNull final RSignature signature2) {
        ...
        for (int i = 0; i < argsTypeName1.size(); i++) {
            ...
            final ClassModuleSymbol leastCommonSuperclass = SqliteRSignatureCacheManager.getLeastCommonSuperclass(set);
